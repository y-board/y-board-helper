...
// BLE-based broadcast radio that mirrors the MakeCode micro:bit radio API.
// Uses BLE manufacturer-specific advertisements for group-filtered broadcasting.
//
// Quick start:
...
// Set transmit power: 0 (≈−12 dBm, lowest) to 7 (≈+9 dBm, highest).
// Default: 6.
void yboard_radio_set_power(uint8_t power);

// Broadcast a number, string, or name+number pair to every device in the same
// group. These functions block for ~200 ms while the BLE packet is on-air.
// Strings are truncated to 16 characters; names to 8 characters.
...
// Callbacks are invoked from the BLE task, not the Arduino main loop —
